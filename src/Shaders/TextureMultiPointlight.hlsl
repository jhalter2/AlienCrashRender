...
//texture stuff starts below here 
...
cbuffer FogParameters : register(b3)
{
	float FogStart;
	float FogRange;
	float4 FogColor;
...
struct VS_OUTPUT
{
	...
	float4 Norm : NORMAL;
	float2 Tex : TEXCOORD; //u,v
};

//--------------------------------------------------------------------------------------
// Vertex Shader
//--------------------------------------------------------------------------------------
VS_OUTPUT VS(float4 Pos : POSITION, float4 nor : NORMAL, float2 t : TEXCOORD)
{
	VS_OUTPUT output;
...
float4 PS(VS_OUTPUT input) : SV_Target
{
	...
	float4 A, D, S;
	//calculate light types
	ComputePointLight(Mater, PntLight[0], input.PosMS, normalize(input.Norm), msDirToEye, A, D, S);
	ambient += A;
	diffuse += D;
	spec += S;
	ComputePointLight(Mater, PntLight[1], input.PosMS, normalize(input.Norm), msDirToEye, A, D, S);
	ambient += A;
	diffuse += D;
	spec += S;
	ComputePointLight(Mater, PntLight[2], input.PosMS, normalize(input.Norm), msDirToEye, A, D, S);
	ambient += A;
	diffuse += D;
	spec += S;
	//calculate light and texture
